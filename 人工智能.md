...
1. 模型复制：对于 N 张 GPU，将第 0 张作为主 GPU，主 GPU 上有一份完整的网络模型，然后复制到其他 GPU 上。
2. 数据分配：一个 batch 的数据先加载到主 GPU 上，然后被分割成 N 份，**由主 GPU 分发给另外 N-1 个 GPU 上**，此时每个 GPU 都有一份数据。
3. 前向传播：每个 GPU 独立进行前向传播（线程并行），计算各自的预测输出。
4. 汇总输出：主 GPU 收集（gather）各个 GPU 到输出，在主 GPU 上计算损失。
5. 反向传播：将计算后到损失分发（scatter）到各个 GPU 上，独立
