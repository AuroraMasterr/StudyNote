...
- 概率越大的事件，提供的信息量越小；概率越小的事件，提供的信息量越大。
- 信息量(自信息)的计算公式：$I(x)=-\log p(x)$

**信息熵（熵）：**

- 用来描述一个事件的不确定性，表示某个事件所有可能发生事件的信息量的期望值。
- $H(x)=-\sum\limits_{i=1}^n{p(x_i)\log(p(x_i))}=\mathbb{E}[\log \frac{1}{p(x)}]$
- 熵的性质：
  - 非负性：$H(X)\ge 0$，当某事情是确定发生的事情时，其熵为 0。
  - 随机变量每次的情况越不确定，其熵值越大，表示变量分布越混乱。
  - 对于有 $n$ 种情况的事件，每种情况的发生概率相等即 $p(x_1)=p(x_2)=...=p(x_n)$ 时，事件的熵最大。

**相对熵（KL散度）：**

- 对于随机变量 $X$ 有两个概率分布 $P(X)$ 和 $Q(X)$，我们可以使用 KL 散度来衡量这两个分布的差异。

  $\begin{align*}D_{KL}(p\|q)&=\sum\limits_{i=1}^n[p(x_i)\log(p(x_i))-p(x_i)\log(q(x_i))]\\&=\sum\limits_{i=1}^np(x_i)\log\frac{p(x_i)}{q(x_i)}\end{align*}$

- 在机器学习中，P 往往用来表示样本的真实分布，Q 用来表示预测的分布，那么 KL 散度表示两个分布的差异，可以作为 Loss 损失值。
- 性质：
  - 非负性：$D_{KL}(p\|q)\ge 0$，当且仅当两分布相同时，KL 散度等于 0.
  - KL 散度不对称，不能用作距离度量。$D_{KL}(p\|q)\ne D_{KL}(q\|p)$
...
**可行性条件（Primal Feasibility Condition）**
...
- 对于参数量为 $\Psi$ 的模型，模型参数占用 $2\Psi$，梯度占用 $2\Psi$，优化器的参数备份、一阶矩估计、二阶矩估计共占用 $(4+4+4)\Psi=12\Psi$，训练至少消耗 $(2+2+12)\Psi=16\Psi$ 显存。
...
- 机器无法直接理解自然语言的文本，我们需要进行文本预处理，第一步就是分词(Tokenization)，执行分词的算法模型称为分词器(Tokenizer)，划分好的词成为词元(Tokens)，每个次元具有相对完整和独立的语义。
- 根据不同的分词算法，我们选取频率/互信息（或者其他权重）最高的若干次组成一个词表(Vocabulary)供模型训练。
...
- 用 Byte 来构建最基础的词表，而不是字符。

- 首先将文本按照 UTF-8 进行编码，每个字符占据 1-4 个 Byte，在 Byte 序列上再使用 BPE 算法，进行 byte level 的相邻合并。

- 在解码阶段，byte 序列可能解码后不是一个合法的字符序列，需要用动态规划算法使得解码出尽可能多的合法字符。

  设 $f(k)$ 表示字符序列 $B_{1:k}$ 最大能解码的合法字符数量，则 $f_k=\max\limits_{i=1,2,3,4}f(k-i)+g(k-i+1,i)$

  如果 $B_{i:j}$ 是一个合法字符则 $g(i,j)=1$，否则 $g(i,j)=0$

**优势：**

- 可以更好地处理跨语言和不常见字符的问题（例如颜文字）
- 相比 BPE 更节省词表空间，同等词表大小效果更好。
- 每个 token 可以获得更充分的训练。
...
- 和 BPE 类似，WordPiece 每次从词表中选择两个子词合并成新的子词。

  和 BPE 的区别在于，BPE 选择频数最高的相邻子词合并，**WorldPiece 选择能够提升语言模型概率最大的相邻子词加入词表**。

- Google 的 Bert 模型在分词的时候使用的是 WordPiece。

**形式化解释：**

- 假设句子 $S=(t_1,t_2,...t_n)$ 由 $n$ 个子词组成，且各个子词之间是独立存在的。

- 句子 $S$ 的语言模型似然值等价于所有子词概率的乘积 $P(S)=\prod\limits_{i=1}^n{P(t_i)}$

  即 $\log{P(S)}=\sum\limits_{i=1}^{n}\log{P(t_i)}$

- 假设把相邻位置的子词 $x$ 和 $y$ 合并成 $z$，此时句子 $S$ 似然值的变化为

  $\log P(z)-(\log P(x)+\log P(y))=\log \frac{P(z)}{P(x)P(y)}$

- 从上述公式中发现，似然值的变化就是两个子词之间的**互信息**。WordPiece 每次选择的两个子词，它们具有**最大的互信息值**，表示两个子词在语言模型上具有较强的相关性且经常以相邻方式同时出现。 
...
<img src="assets/image-20250816161855254.png" alt="image-20250816161855254" style="zoom: 48%;" /><img src="assets/image-20250816161827300.png" alt="image-20250816161827300" style="zoom: 48%;" /> 

<img src="assets/image-20250816162207660.png" alt="image-20250816162207660" style="zoom: 48%;" /><img src="assets/image-20250816162227116.png" alt="image-20250816162227116" style="zoom:48%;" />
...
# scatter: 分发，将输入数据根据第一个维度(通常是 batch_size)划分成多份，发送给多个 GPU
inputs = nn.parallel.scatter(input, device_ids)
# parallel_apply: 并行应用，将分布式的输入数据应用到多个 GPU 上的模型
outputs = nn.parallel,parallel_apply(replicas, inputs)
# gather: 收集，把多个 GPU 得到的数据连接到一起
result = nn.parallel.gather(outputs, output_device)

# torch提供的简单接口，device_ids 指定在哪些 GPU 上训练，output_device 指定输出到哪个 GPU
# torch.nn.DataParallel(module, device_ids=None, output_device=None)
dp_model = nn.parallel.DataParallel(model, device_ids=[0,1,2,3,4,5,6,7])
...
- group：进程组。默认情况下只有一个组，一个 job 就是一个组，也是一个 world。需要更加精细的通信时，可以通过 new_group 接口使用 world 的子集创建新组，用于集体通信等。

- world_size：表示全局并行进程个数

  ```python
  torch.distributed.get_world_size()	# 获取 world_size,在不同进程里都是一样的
  ```

- rank：表示当前进程的序号，用于进程通讯。rank=0 的进程为 master 进程。

  ```python
  torch.distributed.get_rank()		# 获取 rank，每个进程都有自己的序号，各不相同
  ```

- local_rank：表示每台机器上的进程的序号，由 `torch.distributed.launch` 内部指定。

  ```python
  torch.distributed.local_rank()		# 获取 local_rank，用于设置当前进程作用在当前机器的哪个 GPU 上
  ```

- CUDA_VISIBLE_DEVICES：单机多任务的情况下用于控制不同程序可见的 GPU。
...
2. 初始化进程组：通过 `torch.distributed.init_process_group()` 来初始化一个进程组，用于在进程间通信。
   ...
   注：如果需要小组内集体通信，用 new_group 创建子分组
...
- 分布式训练中不同的卡读到的数据应该是不同的，DP 的做法是直接将 batch 切分到不同的卡，这种方式在多机中不可取，因为多机的数据传输会严重影响效率。DDP 利用 sampler 确保 dataloader 只会 load 到整个数据集的一个特定子集，DistributedSampler 为每个进程划分出一部分数据集，以避免不同进程之间数据重复。

- 原型：`torch.utils.data.distributed.DistributedSampler(dataset, num_replicas=None, rank=None)`

  - dataset：进行采样的数据集
  - num_replicas：分布式训练中参与训练的进程数
  - rank：当前进程的 rank 序号

- 与 DDP 配合使用，每个进程传递一个 DistributedSampler 实例作为一个 Dataloader sampler，并加载原始数据集的一个子集作为该进程的输入。

- 在 DP 中，batch_size 设置为单卡的 N 倍，因为一个 batch 被主 GPU 划分成 N 个 minibatch。

  在 DDP 中，batch_size 设置和单卡一样即可，因为各个 GPU 对应的进程独立从磁盘中加载数据。 
...
<img src="assets/image-20250816180518678.png" alt="image-20250816180518678" style="zoom: 33%;" /> 

### PipeDream

[[1806.03377] PipeDream: Fast and Efficient Pipeline Parallel DNN Training](https://arxiv.org/abs/1806.03377)

<img src="assets/image-20250816180920965.png" alt="image-20250816180920965" style="zoom: 67%;" /> 
...
- GPipe 通过将一个 batch 的数据拆分为 micro-batch 实现，并且每一个 batch 完成后会等待全局同步梯度。
...
<img src="assets/image-20250818203523384.png" alt="image-20250818203523384" style="zoom: 50%;" /> 

**ZeRO-powered Data Parallelism，ZeRO-DP**

- 假设模型参数量为 $\Phi$，则显存占用为 $16\Phi$：模型参数（$2\Phi,\mathrm{fp}16$）、模型梯度（$2\Phi,\mathrm{fp}16$）、优化器状态（$12\Phi,\mathrm{fp}32$），Adam 占比为 $75\%$

- 主要有三个优化阶段，分别对应了模型状态中的优化器状态、梯度、模型参数的切分（partition），通常叫做 **ZeRO-1/2/3**，每张卡只保存 $\frac{1}{N}$ 的状态量。

- **ZeRO-1：**优化器状态切分（$P_{os}$）：切分优化器状态到各个计算卡中，在享有与普通数据并行相同通信量的情况下，可降低4倍的内存占用。

  对优化器状态(optimizer states)进行分片，此时每张卡的显存为 $4\Phi+\frac{12\Phi}{N}$，当 $N$ 比较大时，趋向于 $4\Phi$

- **ZeRO-2：** 添加梯度切分（ $P_{os+g}$）：在 $ P_{os}$ 的基础上，进一步将模型梯度切分到各个计算卡中，在享有与普通数据并行相同通信量的情况下，拥有 8 倍的内存降低能力。

- **ZeRO-3：** 添加参数切分（ $P_{os+g+p}$）：在 $P_{os+g}$ 的基础上，将模型参数也切分到各个计算卡中，内存降低能力与并行数量成线性比例，通信量大约有 50% 的增长。

### ZeRO-R

- 优化剩余状态显存占用（Residual states）：激活值、临时缓冲区、无法使用的显存碎片
- 对**激活检查点(Activation Checkpoints)**进行分片储存。
- 预先创建一个固定的缓冲区，训练过程中不再动态创建。如果要传输的数据较小，则多组数据 bucket 后再一次性传输，提高效率
- 显存碎片出现的一大原因是 gradient checkpointing 后不断创建和销毁那些不保存的激活值。解决方法是预先分配一块连续显存，将常驻显存的模型状态和检查点的激活值 （checkpointed activation） 存在里面，剩余显存用于动态创建和销毁不保存的激活值（discarded activation）

### ZeRO-Offload

[ZeRO-Offload: Democratizing Billion-Scale Model Training | USENIX](https://www.usenix.org/conference/atc21/presentation/ren-jie)

- 核心：将一部分计算和储存下放到 CPU 和内存，并且不能让 CPU 和 GPU 之间的通信成为瓶颈，也不能让 CPU 参与过多计算，避免 CPU 计算成为瓶颈。（这种下放，被称为 offload）

### ZeRO-Infinity

[[2104.07857] ZeRO-Infinity: Breaking the GPU Memory Wall for Extreme Scale Deep Learning](https://arxiv.org/abs/2104.07857)
...
- 知识蒸馏由学生模型和教师模型组成，学生模型使用教师模型的输出概率（软目标）和真实标签（硬目标）一起训练。
- **软目标**提供了教师预测的细致视角，它呈现了可能输出的概率分布，而不是单一的正确答案。这些额外的信息帮助学生模型捕捉教师回答中隐含的微妙模式和复杂知识，学生模型可以更好地理解教师的决策过程，保留了教师模型的关键信息，同时使学生模型的训练更加平滑和高效。
